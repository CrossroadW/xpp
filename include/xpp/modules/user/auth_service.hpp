...
public:
    ...
    std::optional<AuthResponse> register_user(const RegisterRequest& req) {
        ...
        auto username_escaped = [&req]() {
            std::string s = req.username;
            std::string out;
            out.reserve(s.size());
            for (char c : s) {
                if (c == '\'') out += "''"; else out += c;
        ...
        // Insert user
        auto email_escaped = [&req]() {
            std::string s = req.email;
            std::string out;
            out.reserve(s.size());
            for (char c : s) {
                if (c == '\'') out += "''"; else out += c;
    ...
    std::optional<AuthResponse> login(const LoginRequest& req) {
        ...
        // Access columns by index: 0=id, 1=username, 2=password_hash, 3=email, 4=avatar_url, 5=is_active
        const auto& row = result[0];
        if (row.size() < 6) {
            xpp::log_error("Invalid result row size");
            return std::nullopt;
        }

        std::string password_hash = row[2];  // password_hash is at index 2
        ...
        User user{
            .id = std::stoll(row[0]),  // id at index 0
            .username = row[1],  // username at index 1
            .email = row[3],  // email at index 3
            .avatar_url = row[4].empty() ? "" : row[4],  // avatar_url at index 4
            .is_active = row[5] == "1" || row[5] == "true"  // is_active at index 5
        };
...
private:
    ...
    std::optional<User> get_user_by_id(int64_t user_id) {
        ...
        // Access columns by index: 0=id, 1=username, 2=email, 3=avatar_url, 4=is_active
        const auto& row = result[0];
        if (row.size() < 5) {
            xpp::log_error("Invalid result row size");
            return std::nullopt;
        ...
        return User{
            ...
            .avatar_url = row[3].empty() ? "" : row[3],  // avatar_url at index 3
            .is_active = row[4] == "1" || row[4] == "true"  // is_active at index 4
        };
    }
};
